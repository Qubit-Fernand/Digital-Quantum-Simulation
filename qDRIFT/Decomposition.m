...
for i = 1:L-1
    ...
    H3 = H3 - J * kron(kron(kron(speye(2^(i-1)),kron(Z, Z)), speye(2^(L-i-1))), speye(2^L));
    % Define the exchange interaction term between adjacent spins (1D chain)
    ...
    H3 = H3 - J * kron(speye(2^L), kron(kron(speye(2^(i-1)),kron(Z, Z)), speye(2^(L-i-1))));    
end 

for i = 1:L
    % Define the exchange interaction term between adjacent spins (inter-chain)
    H1 = H1 - J * kron(kron(kron(speye(2^(i-1)), X), speye(2^(L-i))), kron(kron(speye(2^(i-1)), X), speye(2^(L-i))));
    H2 = H2 - J * kron(kron(kron(speye(2^(i-1)), Y), speye(2^(L-i))), kron(kron(speye(2^(i-1)), Y), speye(2^(L-i))));
